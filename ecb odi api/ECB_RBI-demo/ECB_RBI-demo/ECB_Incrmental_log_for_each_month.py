            ...
            if isinstance(sheet, xlrd.sheet.Sheet):  # Logic for xls files
                ...
                for row in sheet.iter_rows(min_row=1, max_row=sheet.max_row, values_only=True):
                    ...
                    if last_list and "Automatic" in new_list and len(last_list) >= 10:
                        ...
                        insert_data_into_database_49(cursor, last_list, "Automatic")
                        insert_data_database_row = insert_data_database_row + 1
            data_available =insert_data_database_row
            log_list[2] = data_available - 1
...
def del2(cursor):
    cursor.execute("DELETE FROM rbi_ecb WHERE Equivalent_Amount_in_USD='Equivalent Amount in USD ' or Borrower='Equivalent Amount in USD' or Type='Borrower'or Borrower='Borrower' ")
        ...
        for month in remaining_months:
            ...
            if file_name in downloaded_files:
                ...
                try:
                    ...
                    log_list[10] = date_of_scraping
                    status_log = "Failure"
                    reason_log = "Error in insertion part"
                    total = get_data_count(cursor)
                    log_list[4] = total 
                    log_list[1] = status_log
                    ...
                    log_conn.commit()
                    log_list = [None] *11
                    sys.exit(f"error occured in the insert part for this {file_name}")
